...
const KreuzwortraetselPage = () => {
  ...
  const [allCrosswords, setAllCrosswords] = useState([]); // Store all crosswords
  ...
  const crosswordsPerPage = 10;

  // Get current crosswords to display based on pagination
  const getCurrentCrosswords = () => {
    const startIndex = (currentPage - 1) * crosswordsPerPage;
    const endIndex = startIndex + crosswordsPerPage;
    return allCrosswords.slice(startIndex, endIndex);
  };

  // Calculate total pages
  const getTotalPages = () => {
    return Math.ceil(allCrosswords.length / crosswordsPerPage);
  };

  // Get current search results to display based on pagination
  const getCurrentSearchResults = () => {
    if (!searchResults?.edges) return [];
    const startIndex = (searchCurrentPage - 1) * crosswordsPerPage;
    const endIndex = startIndex + crosswordsPerPage;
